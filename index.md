...
Готовые задачи принимаются в виде ссылок на репозиторий на GitHub (GitLab, etc — что душе угодно).

Основной язык программирования: C. В программах, где требуется разбор параметров, делать это через библиотеку getopt. Сборка программы должна осуществляться через make. Для сборки, запуска и тестирования должен использоваться скрипт с именем runme.sh, который при необходимости может вызывать вспомогательные скрипты.

Системные вызовы следует проверять на корректность выполнения; в случае критических ошибок программа должна завершаться с информативным сообщением. Отчет о работе тестов должен сохраняться в файл result.txt и должен содержать краткое описание тестов с описанием ожидаемого результата и вывод фактических результатов тестов.

Временные рамки:
- **ФИИТ-4.** В связи с ранней сессией, для сдачи зачёта автоматом можно сдать три задачи до конца апреля. Для допуска к зачёту - две любые задачи.
- **КБ** (КН и остальные). Для сдачи зачёта автоматом: четыре сделанные вовремя задачи согласно графику. Для допуска к зачёту - две любые задачи.
...
`/bin/sleep 15 /home/user/myinit/in/1 /home/user/myinit/out/1`
...
Файл шаблона для слежения за потомками <a href="fork_waitpid.c">здесь</a>. Шаблон для создания демона на основе примера из книги Робачевского <a href="demonize.c">здесь</a>.

Ключевые функции: fork(), exec(), waitpid(), signal().

**Срок:** до 12.05 включительно.
...
**Срок:** до 31.05 (или до дня перед зачётом).
